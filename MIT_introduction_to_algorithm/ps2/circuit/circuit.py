...
class PriorityQueue:
    ...
    def min_heapify(self, i):
        # O(lgn)
        if(2*(i+1)-1 <len(self.heap) and self.heap[i]>self.heap[2*(i+1)-1]):
            self.heap[i],self.heap[2*(i+1)-1]=self.heap[2*(i+1)-1],self.heap[i]
            self.min_heapify(2*(i+1)-1)
    ...
    def append(self, key):
        ...
        if len(self.heap)==0:
           ...
           while(i>0 and self.heap[(i+1)//2-1]>self.heap[i]):
    ...
    def pop(self):
        ...
        """
        # Instead of directly removing min value, we can switch it with the last value to reduce complexity to lg(n)
        if len(self.heap) == 0:
            return None
        
        # switch min with last value
        self.heap[0], self.heap[-1] = self.heap[-1], self.heap[0]

        # then pop last value
        popped_key = self.heap.pop(-1)

        # then min heapify the value at [0]
        self.min_heapify(0)
